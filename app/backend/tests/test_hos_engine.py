...
from services.types import DutyStatus, EventKind


# ─── Helpers ──────────────────────────────────────────────────────────────────

def assert_24h_invariant(daily_logs):
...
def events_of_kind(timeline, kind: EventKind):
    return [e for e in timeline if e.kind == kind]


def total_drive_hours(timeline) -> Decimal:
    return sum(
        (e.duration_hrs for e in timeline if e.kind == EventKind.DRIVING),
        Decimal("0"),
    )
...
class TestShortHaul:
    ...
    def test_24h_invariant(self):
        assert_24h_invariant(self.logs)

    def test_no_mandatory_rest_injected(self):
    ...
    def test_drive_hours_under_11(self):
...
class TestOTR:
    ...
    def test_multiple_10hr_rests(self):
    ...
    def test_30min_break_injected(self):
        breaks = events_of_kind(self.timeline, EventKind.REST_BREAK)
    ...
    def test_no_single_drive_chunk_exceeds_11hrs(self):
    ...
    def test_multiple_log_days_generated(self):
...
class TestCycleReset:
    ...
    def test_cycle_resets_after_restart(self):
        """After restart, cycle hours should be zero so normal driving proceeds."""
        drive_events = events_of_kind(self.timeline, EventKind.DRIVING)
        total = sum((e.duration_hrs for e in drive_events), Decimal("0"))
        assert total > Decimal("0"), "Driver should be able to drive after reset"

    def test_no_immediate_second_restart(self):
...
class TestProperties:
    ...
    def test_never_crashes_always_24h(
        ...
        # Must always produce at least one log day
        assert len(logs) >= 1

        # The 24h invariant must hold for every generated day
    ...
    def test_no_timeline_gaps_regardless_of_cycle(self, cycle_hours: float):
